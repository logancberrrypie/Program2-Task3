...
An example would be '@@hello' \n")
    ...
    if file[0] == "@" and file[1] == "@":
        ...
        flag = 1
        while flag == 1:
            num_key = int(input("How many keys would you like to enter? "))
            key = []
            for i in range(0,num_key):
                temp = input("What would you like to be keyword number "+str(i+1))
                if choice == "3" or choice == "4":
                    if  not match("^[a-z]*$", temp):
                        print("Invalid input.Please you inputted a character that is not in the 26 letter alphabet! ")
                        break
                key.append(temp)
            flag = 0
